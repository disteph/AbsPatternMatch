...
Record QRenamable :=
  { QRenamablesupport :> Type -> list Sorts -> Type;
    substTerms1 {QVar so l} :
      Term QVar -> QRenamablesupport QVar (so::l) -> QRenamablesupport QVar l;
    qrename {QVar1 QVar2 l} :
      (QVar1 -> QVar2) -> (QRenamablesupport QVar1 l) -> (QRenamablesupport QVar2 l)
  }.

Arguments substTerms1 {_ QVar _ l} _ _.

Fixpoint substTerms {A:QRenamable} {qVar P l} (tl:@TermList (Term qVar) P l)
  := match tl in @TermList _ _ x
          return A qVar x -> A qVar nil with
      | TermNil => fun a => a
      | TermCons r s proof l' tl' => 
        fun a => substTerms tl' (@substTerms1 A qVar s l' r a)
    end.

Variable Atom_d Molecule_d: QRenamable.

Definition Atom QVar := Atom_d QVar nil.
Definition Molecule QVar := Molecule_d QVar nil.

Inductive TypeTree qVar: sTree -> list Sorts -> Type  :=
| TleafP : forall l, Atom_d qVar l -> TypeTree qVar sleafP l
| TleafN : forall l, Molecule_d qVar l -> TypeTree qVar sleafN l
...
Definition namedTypeTree {st qVar qVar'} (qi: qVar -> qVar') 
           (ls: list Sorts) (lq: @UTTermList (Term qVar') ls)
...
Proof.
  ...
  exact (leafP(substTerms lq (Atom_d.(qrename) qi X))).
  exact (leafN(substTerms lq (Molecule_d.(qrename) qi X))).
  exact (dummy).
...
Definition Textends {w st} Delta Gamma 
  :=  let newGam := Contmap (w:=w)
                    (fun i => i)
                    (Atom_d.(qrename) (l := nil) (Context.(qinject) st w))
                    (Molecule_d.(qrename) (l := nil) (Context.(qinject) st w))
                    Gamma
     in
