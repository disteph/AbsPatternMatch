...
mapped to positive (resp. negative) elements of the model *)
  Definition SContext := @DType M.(terms) 
                                   (fun _ _ => M.(primitives))
                                   (fun _ _ => M.(negatives)).
  (* Definition SContext := @DType M.(terms)  *)
  (*                                  {| asfun := fun _ _ => M.(primitives) |} *)
  (*                                  {| asfun := fun _ _ => M.(negatives) |}. *)
...
yet, it is passed as an argument SemContext.  *)
  ...
  Inductive PV
            (A: Molecule qVar)
            (SemContext:forall st l, TypeTree qVar st l -> forall tl, SemTree st tl -> Prop)
  : M.(positives) -> Prop :=
    pv : forall p Delta v,
...
interpretation of any molecule B in Delta *)
  ...
  Definition SemCont_aux (sigma:@tvaluation M qVar)
             ...
             st
    := Treelift M.(sortsI) (ToFamily sigma (M.(atomI) qVar sigma)) (ToFamily sigma f) (st:=st).
...
orthogonality relation be closed "under anti-reduction" *)
  ...
  Arguments SemP {M w} _ _.
  Arguments SemSub {M w} _ {st} _ _.
  Arguments SemN {M w} _ _.
  Arguments SemOptionC {M w} _ _.
  Arguments SemC {M w} _ _.
...
semantic contexts *)
  ...
  Hypothesis HValues: forall qVar sigma st (v:SemTree st nil) Delta,
                        SemCont (qVar:=qVar) sigma nil Delta nil v -> treevalue M v.
  ...
  Theorem adequacy:
    forall (w:World) (Gamma:TContext w),
      (forall pt A,
         TypingPos Gamma pt A -> forall rho, compat Gamma rho -> SemPos rho.(readq) A (SemP rho pt))
      /\ (forall l st (v:TermTree st) Delta tl,
           TypingSub Gamma v Delta tl
           ->  forall rho, compat Gamma rho -> let tls := SemTermList M w.(QVar) rho.(readq) tl in
                                SemCont rho.(readq) l Delta tls (SemSub rho v tls))
      /\ (forall nt A,
           TypingNeg Gamma nt A ->  forall rho, compat Gamma rho -> SemNeg rho.(readq) A (SemN rho nt))
      /\ (forall c,
           TypingOptionCommand Gamma c
           ->  forall rho, compat Gamma rho -> exists csem, (SemOptionC rho c) = Some csem /\ M.(orth) csem)
      /\ (forall c,
           TypingCommand Gamma c ->  forall rho, compat Gamma rho -> M.(orth) (SemC rho c))
  .
    apply typing_ind;intros;
    [
      (* Typing a positive *)
      by apply SemPosCont; apply (pv _ _ _ _ _ Delta) =>//; apply H
      ...
      | by unfold SemCont => //=;
        apply is_from_terms; [rewrite/compattval; apply H0 | apply H]
      ...
      | by unfold SemCont => //=;
        split; [apply (sortscompat M (readq Gamma) (readq rho) (proj1 H0)) | apply H]
      (* Typing a negative *)
      | simpl
      ...
      (* Typing a cut *)
      | by unfold SemCont => //=;unfold SemNeg, ortho in H;auto

      (* Typing a select *)
      | by unfold SemC;unfold compat, SemNeg, ortho in H0;apply H0; auto
    ].
    ...
    move :
      (Context.(corr) (w:=w) (st:=PatTree p)
                      M.(sortsI)
                          (fun qVar _ sigma => ToFamily sigma (atomI sigma))
                          (fun qVar _ sigma => ToFamily sigma (SemNeg sigma))
                          Gamma rho Delta v H4 H0
      ).
    clear H0 => H0.
    move: (proj2 (ToFamilyNil _ _ _) H0); clear H0 => H0.
    done.

    elim: H6 => csem ; elim => H6 H7.
    move : H6.
    remember (f p) as u.
    move: Hequ.
    elim: u => //.
