...
Lemma EvalDec : 
  forall w (rho:AM.(EContext) w) st, @TermDec LAF w st ->  @EDec st.
Proof.
  move => w rho.
...
Section Normalisation.
  ...
  Lemma SemEvalDec {w} : forall rho st (v:@TermDec LAF w st), 
  ...
  Lemma Rem61 : forall w (c:@Command LAF w) rho, ECom2ETri << c | rho >> (SemC (M:= HeadStruct) rho c).
  Proof.
  ...
  Lemma Rem61' : forall w (c:@Command LAF w) rho x, 
                   ECom2ETri << c | rho >> x
