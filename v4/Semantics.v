...
Section Semantics.
  ...
  Context `(LAF:@LAFs SV QV TV SoV SoRV ssuper AV MV is_eqV super wV TCV PatV PatDecV PTypedV).
...
Print World.
    ...
    Class ModelStructures STerms Valuations SLab SPos SNeg orth SContexts tild SemTerms I := 
      {
        STerms   : Type := STerms;
        Valuations : QWorld LAF -> Type := Valuations;
        SLab     : Type := SLab;
        SPos     : Type := SPos;
        SNeg     : Type := SNeg;
        orth     : SNeg*SPos -> Prop := orth;
        SContexts: Contexts SLab SNeg STerms Valuations := SContexts;
        tild (p: Patterns LAF)   : @Dec STerms SLab SNeg (PatDec p) -> SPos := tild p;
        SemTerms : forall {qLab: QWorld LAF}, Valuations qLab -> Terms qLab -> STerms := SemTerms;
        I : forall{w: World LAF}, SContexts w -> Reifiable w -> SNeg := I
      }
    .

    Global Arguments tild {_ _ _ _ _ _ _ _ _ _ _} p _.

    (* We now suppose we have a model structure *)

    Context `(M: ModelStructures).

    (* A useful abbreviation for semantical decompositions, which will
  interpret term decompositions *)

    Definition SDec := @Dec STerms SLab SNeg.

    Definition STList := AList (QS := LAF) STerms.
    ...
    Definition SemN w (rho: SContexts w) (tm : Neg) := 
      match tm with
        | rei f => I rho f
      end
    .
    ...
    Fixpoint SemDec w (rho: SContexts w) {st} (v : TermDec st) {struct v} :=
      match v in @TermDec _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ s return SDec s with
    ...
    Definition SemP w (rho:SContexts w) (pt : Pos) := 
      match pt with
...
element) of the model, that may or may not be orthogonal *) 
    ...
    Definition SemC w (rho:SContexts w) (c : Command) :=
      match c with
    ...
    Definition SemOptionC w (rho:SContexts w)(oc : OptionCommand) :=
      match oc with
        | some c => Some(SemC (rho:SContexts w) c)
        | none => None
      end
    .
...
We formalise this. *)
    ...
    Inductive  belong l
    : forall l' st, TypingDec (QST := LAF) st l' -> Molecule (q := LAF) l -> Prop
      :=
    ...
    Inductive relation : DPair (Molecule (q := LAF))-> DPair (Molecule (q := LAF))-> Prop :=
      relation_base:
    ...
    Definition Pard `{M: ModelStructures} {A B} 
               (f: forall a:DPair A, STList M (ex1 a) -> B)
               {qLab} sigma a
      := f {{ ex1 a, getA (qLab := qLab) a }} (SemTermList sigma (getTerms a)).
    
    Class RealisabilityAlg
           `(MS: ModelStructures)
           welf 
           (SemSorts : Sorts LAF -> STerms -> Prop)
           (SemSoCont: forall {qLab}, SoContexts qLab -> Valuations qLab -> Prop)
           (SemSoCompat:
              forall qLab Sigma r s sigma, 
                SortingRel (qLab := qLab) Sigma r s
                -> SemSoCont Sigma sigma
                -> SemSorts s (SemTerms sigma r)
           )
           (SemAtom: forall a:DPair Atom, STList MS (ex1 a) -> SLab -> Prop)
           (SemAtom_eq:
              forall qLab sigma a b p, 
                is_eq a b
                -> Pard (qLab := qLab) (@SemAtom) sigma a p
                -> Pard (@SemAtom) sigma b p)
      :=
      {
        welf: well_founded relation := welf;
        SemSorts  := SemSorts;
        SemSoCont qLab   := SemSoCont (qLab := qLab);
        SemSoCompat qLab := SemSoCompat qLab;
        SemAtom  := SemAtom;
        SemAtom_eq := SemAtom_eq
      }.

    Global Arguments SemAtom {_ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _} a _ _.

    (* We now assume we have a realisability algebra *)

    Context `(M: RealisabilityAlg (MS := MS)).
    ...
    Definition ortho P tm :=
      forall tp: SPos, P tp -> orth (tm,tp).
...
yet, it is passed as an argument SemContext.  *)
    ...
    Inductive PV
              ...
              (SemTDec: forall st, TypingDec st (ex1 A)
                              -> forall tl:STList MS (ex1 A), SDec MS st -> Prop)
              (tl: STList MS (ex1 A))
    : SPos -> Prop :=
      pv : forall p Delta v,
...
interpretation of any molecule B in Delta *)
    ...
    Definition SemTDec_aux
               (f : forall (A:DPair Molecule), STList MS (ex1 A) -> SNeg -> Prop)
               l
               st
...
positive interpretation of any molecule B smaller than A *)
    ...
    Definition SemPos_aux
               (A: DPair Molecule)
               (f : forall B, relation B A -> STList MS (ex1 B) -> SPos -> Prop)
    ...
    Lemma F_ext_aux:
      forall (x:DPair Molecule)
        (f g: forall y:DPair Molecule, 
                relation y x -> STList MS (ex1 y) -> SPos -> Prop),
        (forall (y:DPair Molecule) (p:relation y x) tl d, f y p tl d <-> g y p tl d) 
        -> forall st l (Delta:TypingDec st l),
          forall tl' (v : SDec MS st),
            SemTDec_aux (fun (B : DPair Molecule) tl (tm : SNeg) =>
