...
Definition ParMol qVar := {l: list Sorts & Family Molecule qVar l}.
...
Section TypeSemantics.
  ...
  Record ModelRaw :=
    { model4PTerms :> Model4PTerms;
      ...
      atomIstable : forall qvar qvar' (a:Atom qvar) sigma tau (ren: qvar -> qvar')
...
yet, it is passed as an argument SemContext.  *)
  ...
  Inductive PV
            ...
            (SemContext:forall st l, TypeTree qVar st l
                                -> forall tl:@UTTermList qVar l, SemTree st -> Prop)
  : M.(positives) -> Prop :=
    pv : forall p Delta v,
  ...
  Proof.
    ...
    induction st;
      ...
      + (* move => [Sigma CV tll tm] H1; apply is_from_terms =>//. *) 
        move => H1.
        assert (belong sleafN (TleafN f) (substTerms tl f)).
        apply rel_leaf.
        assert (relation (substTerms tl f) A).
        apply H => //=.
        by exists H2.
...
semantic contexts *)
  ...
  Lemma SemPos2Treelift  qVar qVar' (sigma:tvaluation qVar) (tau:tvaluation qVar') (ren:qVar -> qVar') st qnew l Delta tl tl' v 
  (pi: forall xq, tau (ren xq) = sigma xq)
  : SemCont (M := M)(st := st) sigma l Delta tl v
    -> Treelift M.(sortsI) (atomI tau) (SemNeg tau) 
                          (namedTypeTree ren l tl' Delta qnew)
                          v.
  Proof.
    induction st; dependent induction v; dependent induction Delta; dependent induction qnew =>//=.
    rewrite/substTerms.

  Lemma SemNegstable : forall qvar qvar' (a:Molecule qvar) sigma tau (ren: qvar -> qvar')
                         (pi: forall xq, tau (ren xq) = sigma xq)
                         p,
                         SemNeg (M:=M) sigma a p <-> SemNeg tau (Molecule.(qrename) ren a) p.
  Proof.
    admit.
  Qed.
  ...
  : compat sigma Gamma rho -> compat tau (Contmap (w:=w)
                                 (fun i => i)
                                 (Atom.(qrename) renaming)
                                 (Molecule.(qrename) renaming)
                                 Gamma) rho.
  Proof.
    rewrite/compat/Contlift/Contmap;elim => H1 [H2 H3] /=.
                                              split => //;split.
    move => xp.
    rewrite <- M.(atomIstable) => //.
    move => xn.
    rewrite <- SemNegstable => //.
  Qed.

  Lemma substIgnoresTypes_base {A qVar Sigma l} (tl:TTermList Sigma l) X: 
    substTerms_base (TermListMap tl) X = substTerms_base (A := A)(qVar := qVar) tl X.
  Proof.
  ...
  Proof.
    rewrite/substTerms/substTermsNU.
    apply substIgnoresTypes_base.
  Qed.
    ...
    forall (w:World) (Gamma:TContext w),
      (forall pt A,
         ...
         ->  forall rho, compat rho.(readq) Gamma rho
                 -> (forall st (v:SemTree st) Delta,
                      SemCont rho.(readq) nil Delta TermNil v -> treevalue M v)
