...
Section TypeSemantics.
  ...
  Record ModelRaw :=
    { model4PTerms :> Model4PTerms;
      ...
      atomI : forall qVar l, @tvaluation model4PTerms qVar
                        -> TermFamily Atom qVar l
  ...
  Definition compatval {qVar} Sigma sigma := forall qvar:qVar, M.(sortsI) (Sigma qvar) (sigma qvar).
...
It is a list of semantical terms *)
  ...
  Fixpoint SemTermList {qVar Sigma} (sigma: tvaluation qVar) {l} (tl:@TermList qVar Sigma l)
    := match tl with
        | TermNil => nil
  ...
  Lemma WSSemTermList {qVar Sigma} {sigma: tvaluation qVar} {l} (tl:@TermList qVar Sigma l)
  : compatval Sigma sigma -> SemSortedlist l (SemTermList sigma tl).
  Proof.
    induction tl => //=.
  ...
  Definition rightArgs {qVar} (lA:ParMol qVar) tl := let (l,_) := lA in SemSortedlist l tl.
...
yet, it is passed as an argument SemContext.  *)
  ...
  : M.(positives) -> Prop :=
    pv : forall p Delta v,
           ...
           -> PV qVar sigma A SemContext (M.(tild) p v).

  Inductive PVP qVar Sigma (sigma : @tvaluation M qVar) (CV:compatval Sigma sigma)
            f
            l
            (A: TermFamily Molecule qVar l)
  : forall tl, SemSortedlist l tl -> M.(negatives) -> Prop
    := is_from_terms tl tm:
        f (substTerms tl A) tm
        -> PVP qVar Sigma sigma CV f l A (SemTermList sigma tl) (WSSemTermList tl CV) tm.
...
interpretation of any molecule B in Delta *)
  ...
  : forall tl, SemSortedlist l tl -> @SemTree M st tl -> Prop.
    ...
    exact (M.(atomI) qVar l sigma X tl H X0).
    exact ((PVP qVar Sigma sigma CV f) l X tl H X0).
    exact True.
  ...
  Definition SemPos_aux
    ...
    := 
      PV qVar sigma A 
         (SemCont_aux 
            qVar Sigma sigma CV
            (fun B tm => exists h:relation (Sigma:=Sigma) B A, ortho (f B h) tm)
         )
  .
...
the well-founded relation on molecules. *)
  ...
  Definition SemPos qvar Sigma sigma CV := Fix welf _ (SemPos_aux qvar Sigma sigma CV).
...
interpretation of A.  *)
  ...
  Definition SemNeg
             qVar
             Sigma
             (sigma: tvaluation qVar)
             CV
             (A: Molecule qVar)
    := 
      ortho (SemPos qVar Sigma sigma CV A).
...
passed as a parameter, it is the actual interpretation SemNeg *)
  ...
  Definition SemCont qVar Sigma (sigma:tvaluation qVar) CV {st}
    := SemCont_aux qVar Sigma sigma CV (SemNeg qVar Sigma sigma CV) st.
...
before that we need the following two lemmas. *)
  ...
  Lemma F_ext_aux qVar Sigma sigma CV:
    forall (x:Molecule qVar)
      ...
      -> forall st l (Delta:TypeTree qVar st l),
          forall tl' HWS',
          forall v : SemTree st tl',
            SemCont_aux qVar Sigma sigma CV
  ...
  Lemma F_ext qVar Sigma sigma CV :
    forall (x:Molecule qVar)
      (f g: forall y:Molecule qVar, relation y x -> M.(positives) -> Prop),
      (forall (y:Molecule qVar) (p:relation y x) d,
         f y p d <-> g y p d) 
      -> forall d, SemPos_aux qVar Sigma sigma CV x f d -> SemPos_aux qVar Sigma sigma CV x g d.
  Proof.
    move => A f g H d.
    unfold SemPos_aux.
    elim.
    intros.
    apply (pv qVar sigma A _ p Delta v) => //.
    move : v H1 ; clear H0 d.
    apply (F_ext_aux _ _ _ _ A) =>//. 
  Qed.

  Lemma Fix_F_inv qVar Sigma sigma CV :
    forall (x:Molecule qVar) d (r s:Acc relation x),
      Fix_F (fun _ => M.(positives) -> Prop) (SemPos_aux qVar Sigma sigma CV) r d 
      <-> Fix_F (fun _ => M.(positives) -> Prop) (SemPos_aux qVar Sigma sigma CV) s d.
  Proof.
    ...
    split.
    apply (F_ext qVar Sigma sigma CV _
                 (fun (y : Molecule qVar) (p : relation y x)  =>
                    Fix_F _
                          (SemPos_aux qVar Sigma sigma CV) (Acc_inv r p))) => //=.
                                                             auto.
    apply (F_ext qVar Sigma sigma CV _
                 (fun (y : Molecule qVar) (p : relation y x)  =>
                    Fix_F _
                          (SemPos_aux qVar Sigma sigma CV) (Acc_inv s p))) => //=.
                                                             auto.
  Qed.
...
but with the actual SemPos taking the place of the parameter f *)
  ...
  Lemma SemPos_Fix qVar Sigma sigma CV :
    forall A tp, 
      SemPos qVar Sigma sigma CV A tp 
      <-> PV qVar sigma A 
           (SemCont_aux 
              qVar Sigma sigma CV
              (fun B tm
               => exists h:relation (Sigma:=Sigma) B A, ortho (SemPos qVar Sigma sigma CV B) tm)
           )
           tp.
  Proof.
    move => A tp; unfold SemPos; unfold Fix in |- *.
    rewrite <- Fix_F_eq.
    split; apply (F_ext qVar Sigma ); intros; apply Fix_F_inv.
  Qed.
...
of the definition of SemPos) *)
  ...
  Lemma SemPosCont_aux1 qVar Sigma sigma CV: 
    forall A st l (Delta:TypeTree qVar st l) tl HWS (v:SemTree st tl), 
      ...
      -> SemCont_aux qVar Sigma sigma CV
                    (fun (B : Molecule qVar) (tm : negatives M) =>
                       ortho (SemPos qVar Sigma sigma CV B) tm) 
                    st l Delta tl HWS v      
      -> SemCont_aux qVar Sigma sigma CV
                    (fun (B : Molecule qVar) (tm : negatives M) =>
                       exists _ : relation (Sigma:=Sigma) B A, ortho (SemPos qVar Sigma sigma CV B) tm)
                    st l Delta tl HWS v.
  Proof.
    move => A.
    induction st;
  ...
  Lemma SemPosCont_aux2 qVar Sigma sigma CV: 
    forall A st l (Delta:TypeTree qVar st l) tl HWS (v:SemTree st tl), 
      SemCont_aux qVar Sigma sigma CV
                    (fun (B : Molecule qVar) (tm : negatives M) =>
                       exists _ : relation (Sigma:=Sigma) B A, ortho (SemPos qVar Sigma sigma CV B) tm)
  ...
  Lemma SemPosCont_aux qVar Sigma sigma CV: 
    forall A tp, 
      PV qVar sigma A 
         (SemCont_aux 
            qVar Sigma sigma CV
            (fun B tm => ortho (SemPos qVar Sigma sigma CV B) tm)
         )
         tp      
      <-> PV qVar sigma A 
           (SemCont_aux 
              qVar Sigma sigma CV
              (fun B tm => exists h:relation (Sigma:=Sigma) B A, ortho (SemPos qVar Sigma sigma CV B) tm)
           )
           tp.
  Proof.
    intros.
    split => H;elim H;clear H;intros; apply (pv qVar sigma A _ p Delta v) => //=.
    + assert (forall B, belong (Sigma:= Sigma) (PatTree p) Delta B -> relation (Sigma:= Sigma) B A).
      ...
      apply (SemPosCont_aux2 _ _ _ _ A) =>//.
  Qed.
...
lemmas *)
  ...
  Theorem SemPosCont qVar Sigma sigma CV : forall A tp, @SemPos qVar Sigma sigma CV A tp
                                           <-> PV qVar sigma A (@SemCont qVar Sigma sigma CV) tp.
  Proof.
    intros;split; intro.
...
semantic contexts *)
  ...
  Definition compat {w} (sigma:tvaluation M) := Contlift sigma (SemNeg M sigma) (w := w).
